...
var loadConfigFile = function (type) {
	...
	if(type == 'user') {
		var url = '/admin/filemanager/scripts/filemanager.config.js';
	} else {
		var url = '/admin/filemanager/scripts/filemanager.config.js.default'
	}
    console.log(url);
    $.ajax({
        'async': false,
...
var capabilities = config.options.capabilities || new Array('select', 'download', 'rename', 'move', 'delete', 'replace', 'selectImage');

// Get localized messages from file 
// through culture var or from URL
if($.urlParam('langCode') != 0 && file_exists ('/admin/filemanager/scripts/languages/'  + $.urlParam('langCode') + '.js')) config.options.culture = $.urlParam('langCode');

var lg = [];
$.ajax({
  url: '/admin/filemanager/scripts/languages/'  + config.options.culture + '.js',
  async: false,
  dataType: 'json',
...
function has_capability(data, cap) {
	if (data['File Type'] == 'dir' && (cap == 'download' || cap == 'replace' || cap == 'selectImage')) return false;
...
var bindToolbar = function(data) {
	...
	if (!has_capability(data, 'selectImage')) {
		$('#fileinfo').find('button#selectImage').hide();
	} else {
        $('#fileinfo').find('button#selectImage').click(function () { selectImage(data); }).show();
	}
...
// passes file path, dimensions, and alt text back to the
// opening window. Triggered by clicking the "Select Image" 
// button in detail views or choosing the "Select Image"
// contextual menu option in list views. 
// NOTE: closes the window when finished.
var selectImage = function(data) {
	if(config.options.relPath !== false ) {
		var url = relPath + data['Path'].replace(fileRoot,""); 
	} else {
		var url = relPath + data['Path'];
	...
	console.log('stored url of selected Image is '+url);
	window.parent.jQuery('#myModal').modal('hide');
};
...
function getContextMenuOptions(elem) {
	...
	if (!($('#' + optionsID).length)) {
		...
		if (!elem.hasClass('cap_selectImage')) $('.selectImage', newOptions).remove();
...
var getFileInfo = function(file) {
	...
	if($.inArray('selectImage', capabilities)  != -1) template += '<button id="selectImage" name="selectImage" type="button" value="Select Image">' + lg.selectImage + '</button>';
...
var getFolderInfo = function(path) {
	...
	$('#fileinfo').html('<img id="activity" src="/admin/filemanager/images/wait30trans.gif" width="30" height="30" />');
...
$(function(){
	...
	if(!config.options.fileRoot) {
		// fileRoot = './public/admin/filemanager/userfiles/';
		fileRoot = './public/images/';
	 //fileRoot = './public/' + document.location.pathname.substring(1, document.location.pathname.lastIndexOf('/') + 1) + 'userfiles/../../../images/';
	...
	if(config.options.autoload == true) {
		...
		$('#itemOptions a[href$="#selectImage"]').append(lg.selectImage);
	...
	/** load searchbox */
	if(config.options.searchBox === true)  {
		loadJS("/admin/filemanager/scripts/filemanager.liveSearch.min.js");
	} else {
		$('#search').remove();
	}
